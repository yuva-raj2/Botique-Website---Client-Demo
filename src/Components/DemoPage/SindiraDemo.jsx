...
export default function SindiraDemo() {
  ...
  const products = [
    ...
    {
      id: 2,
      name: "Indowestern Flared Motif Skirt",
  ...
  return (
    <div style={{ 
        width: '100%',
        maxWidth: '450px', // Perfect mobile viewport constraint
        background: '#FBFBFA', 
        minHeight: '100vh', 
        paddingBottom: '60px', 
      ...
      <div style={{ background: '#111', color: '#FFF', padding: '10px', textAlign: 'center', fontSize: '11px', letterSpacing: '1px', fontWeight: '500' }}>
        ✨ PROTOTYPE STOREFRONT FOR SINDIRA • GLOBAL SHIPPING
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 50, padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #EFEFEF' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '20px', fontWeight: '700', letterSpacing: '1.5px', color: '#111' }}>SINDIRA</h1>
          <p style={{ margin: 0, fontSize: '11px', color: '#777', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Indowestern for the modern woman</p>
        </div>
      ...
      {/* Pitch Intro Alert Box (Only visible for the client to realize your value) */}
      ...
      <div style={{ padding: '0 16px' }}>
        {products.map((product) => (
          <div key={product.id} style={{ background: '#FFF', borderRadius: '16px', border: '1px solid #EAEAEA', overflow: 'hidden', marginBottom: '24px', transition: 'all 0.3s ease' }}>
      ...
      <div style={{ padding: '0 20px', textAlign: 'center', color: '#888', fontSize: '12px' }}>
